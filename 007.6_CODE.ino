...
const int trigger_pin_2 = 10; // Pin del Arduino conectado al trigger del sensor ultrasónico de la bandera
const int echo_pin_2 = 11; // Pin del Arduino conectado al echo del sensor ultrasónico de la bandera
...
const int next_button_pin = A3; //Botón para pasar al siguiente paso
const int on_button_pin = A2; // Botón para encender el electroiman
const int off_button_pin = A1; // Botón para apagar el electroiman
const int one_two_button_pin = A0; // Botón para encender seleccionar "1" o "2"
...
int initial_step = 0;
int anvil = 1;
const int max_distance = 280; /// Distancia máxima que queremos medir en el ultrasonico #1 (en centímetros).

#define ALTURA_ACTIVACION 60 // Altura para activar el buzzer y brazo del hc sr04 del headform
...
int initial_pos_laser = 25; //posición inicial del brazo laser
int initial_pos_hc_head = 170; //posición inicial del brazo del HC-SR04 del headform
int final_pos_laser = 122; //posición final del brazo laser, alineado al centro del anvil
int final_pos_hc_head = 78; //posición final del brazo del HC-SR04 del headform, alineado al centro del anvil

const int hc_flat = 17; //Distancia en cm del sensor ultrasonico del headform con la superficie del yunque plano
const int hc_sphere = 16; //Distancia en cm del sensor ultrasonico del headform con la superficie del yunque semiesferico
const int hc_2_ground = 3; //Distancia(cm) fija del ultrasonico de la bandera (medido desde su parte frontal) con el suelo
...
#define NUM_READINGS 2 // Número de lecturas para promediar
...
int current_pos_laser = initial_pos_laser;  // variable global

long dd = 0;  // ← variable global
...
void setup() {
  ...
  //pinMode(trigger_pin_2, OUTPUT); //pin como salida
  //pinMode(echo_pin_2, INPUT);  //pin como entrada
  //digitalWrite(trigger_pin_2, LOW);//Inicializamos el pin con 0
...
if (stepActive && number_step != 4 && number_step != 9 && number_step != 10) {
  showStepWithBlinkingUnderscore(number_step);
}
...
void lcd_setup(){
  ...
  lcd.print("USUARIO TITON AL"); // print message at the 2nd row
  lcd.setCursor(0, 2);            // move cursor to the 3rd row
  lcd.print("IMPACTUN X, MAQUINA"); // print message at the 3rd row
  lcd.setCursor(0, 3);            // move cursor to the 4th row
  lcd.print("DE IMPACTOS VERTICAL");   // print message the 4th row
}
...
void step_3(){
  lcd.setCursor(2, 0);            // move cursor the 1st row
  lcd.print("'NEXT' SI ESTA"); 
  lcd.setCursor(1, 1);            // move cursor to the 2nd row
  ...
  lcd.setCursor(3, 3);            // move cursor to the 4th row
  lcd.print("PRESIONAR 'ON'");   // print message the 4th row
  update_electromagnet();
}
...
void step_4(){
  ...
  lcd.setCursor(3, 3);            // move cursor to the 4th row
  lcd.print("CABEZA Y CASCO");   // print message the 4th row
}
...
void step_10(){
  ...
  lcd.setCursor(0, 1);            // move cursor to the 2nd row
  lcd.print("DE 'ON' & 'OFF' PARA");   // print message at the 2nd row
  lcd.setCursor(0, 2);            // move cursor to the 3rd row
  ...
  if (off_button == 1 && on_button == 1 && !countdown_active) {
    delay(150); // Pequeño debounce
    ...
    delay(1000); // Espera el resto del segundo antes del siguiente número
  }
...
void main_hcsr04(){
  ...
  long average_3 = total_3 / NUM_READINGS;
  ...
  if(average_3 < ALTURA_ACTIVACION){
    hc_head.write(initial_pos_hc_head);
  }
  else{
...
void flag_hcsr04(){
  ...
  // Calcular el promedio de las lecturas
  long average_2 = total_2 / NUM_READINGS;

  /*long tt; //timepo que demora en llegar el eco
 
  digitalWrite(trigger_pin_2, HIGH);
  delayMicroseconds(10);          //Enviamos un pulso de 10us
  digitalWrite(trigger_pin_2, LOW);
  
  tt = pulseIn(echo_pin_2, HIGH); //obtenemos el ancho del pulso
  dd = tt/59;             //escalamos el tiempo a una distancia en cm*/
...
/*void flag_velocimeter(){
  ...
  // Si la distancia es 2 cm y el buzzer no ha sonado antes, lo activamos
  if (average_3 <= ALTURA_MAXIMA && average_3 >= 0 && !buzzer_activated) {
    digitalWrite(buzzer, HIGH);
    delay(50);  // Suena por 0.05 segundos
    digitalWrite(buzzer, LOW);
...
void showStepWithBlinkingUnderscore(int step) {
  static bool underscoreVisible = true;
  static unsigned long lastToggleTime = 0;
  const unsigned long interval = 500; // tiempo de parpadeo (ms)

  // Alternar visibilidad del guion bajo
  ...
  // Letra 'P'
  data[0] = SEG_A | SEG_B | SEG_E | SEG_F | SEG_G;

  // Guion bajo (si está visible)
  data[1] = underscoreVisible ? SEG_D : 0x00;

  // Dígitos (los pasos deben ser menores a 100)
  int tens = step / 10;
  int units = step % 10;
